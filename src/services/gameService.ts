...
class GameService {
  ...
  async createGameRoom(
    ...
    try {
      ...
      // Validate bonus system
      if (gameOptions?.bonusEnabled && (!gameOptions.bonusAmount || gameOptions.bonusAmount <= 0)) {
        throw new Error('Bonus amount must be greater than 0 when bonus is enabled');
      }

      // Validate invitation system
      if (gameOptions?.invitationEnabled && !gameOptions.invitationCode) {
        throw new Error('Invitation code is required when invitation system is enabled');
      }
      ...
      const gameRoom: Omit<GameRoom, 'id'> = {
        ...
        ...(gameOptions?.bonusEnabled ? {
          bonusSystem: {
            enabled: true,
            amount: gameOptions.bonusAmount || 0
          }
        } : {}),
        ...(gameOptions?.invitationEnabled ? {
          invitationSystem: {
            enabled: true,
            code: gameOptions.invitationCode || ''
          }
        } : {}),
        ...(gameOptions?.scheduledTime ? {
          scheduledTime: gameOptions.scheduledTime
        } : {})
      };
  ...
  async joinGameRoom(gameRoomId: string, player: Player, invitationCode?: string): Promise<void> {
    try {
      ...
      if (gameData.invitationSystem?.enabled) {
        if (!invitationCode) {
          throw new Error('Invitation code is required to join this game');
        }
        if (invitationCode !== gameData.invitationSystem.code) {
  ...
  async getGameRoom(gameRoomId: string): Promise<GameRoom | null> {
    try {
      ...
      handleFirestoreError(error, 'get game room');
      throw error;
    }
  }
  ...
  async declareWinner(gameRoomId: string, winnerId: string, winPattern: string, winPercentage: number): Promise<void> {
    try {
      ...
      const gameData = gameRoomSnap.data() as GameRoom;
      let winAmount = gameData.prizePool * winPercentage;

      // Add bonus if enabled
      if (gameData.bonusSystem?.enabled) {
        winAmount += gameData.bonusSystem.amount;
      }
      ...
      await addDoc(collection(db, 'transactions'), {
        ...
        amount: winAmount,
        status: 'completed',
        description: `Bingo win: ${winPattern}${gameData.bonusSystem?.enabled ? ' + Bonus' : ''}`,
        createdAt: serverTimestamp(),
        metadata: {
          gameId: gameRoomId,
          winPattern: winPattern,
          winPercentage: winPercentage,
          baseAmount: gameData.prizePool * winPercentage,
          bonusAmount: gameData.bonusSystem?.enabled ? gameData.bonusSystem.amount : 0
        }
      });
